...
class Connection extends EventEmitter {
  constructor(opts) {
    ...
    // 如果没有设置好stream
    // 也没有 socker连接 就通过net 创建一条连接
    console.log("初始化链接，目的是为了创建mysql长链接通信，给stream赋值");
    ...
    this.stream.on('data', data => {
      console.log('-----------------------------------------------------');
      console.log((++i) + ' stream string',data.toString('utf8'));
      console.log(' stream buffer',data);
      console.log('-----------------------------------------------------');
  ...
  query(sql, values, cb) {
    ...
    console.log("序列化sql");
    const format = this.format(cmdQuery.sql, cmdQuery.values !== undefined ? cmdQuery.values : []);
    const rawSql = format;
    cmdQuery.sql = rawSql;
    console.log("添加指令");
