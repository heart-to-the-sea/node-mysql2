...
class ConnectionConfig {
  constructor(options) {
    console.group('ConnectionConfig')
    if (typeof options === 'string') {
      console.log('如果传入的是字符串，则将其序列化为配置对象');
      options = ConnectionConfig.parseUrl(options);
    } else if (options && options.uri) {
      console.log("如果传入的是对象但是包含url，就将url解析为配置对象，并替换原来的配置对象的属性");
    ...
    console.log("判断传入的参数是否是合法的，自定义参数会被抛出警告");
    ...
    console.groupEnd('ConnectionConfig')
  }
